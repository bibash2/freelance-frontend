<script lang="ts">
  ...
  import { createEventDispatcher } from 'svelte';
    ...
    $inspect(props);
    const {fromName, message, phone, email, time, consumerAvatar, isOnline, isNew, type, user, dateTime,address} = props;
    ...
    <div class="flex items-start gap-4 h-full">
      ...
      <div class="flex-1 min-w-0">
        <div class="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-2 mb-2">
          ...
          <span class="text-xs text-gray-500 whitespace-nowrap">{new Date(dateTime).toLocaleString()}</span>
        </div>
        ...
        <div class="space-y-2">
          ...
          {#if user.email}
            <div class="flex items-center gap-2 text-sm">
              ...
              <a href={`mailto:${user.email}`} class={`text-gray-700 hover:${colorClasses.text} hover:underline truncate`}>
                {user.email}
              </a>
            </div>
            <div class="flex items-center gap-2 text-sm">
